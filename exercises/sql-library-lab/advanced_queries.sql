...
SELECT title FROM books JOIN authors ON books.author_id = authors.id WHERE books.publication_date > 1930 AND authors.nationality = 'Argentina';

-- Find all books written before 1980 by authors not from the US.

SELECT title FROM books JOIN authors ON books.author_id = authors.id WHERE books.publication_date > 1980 AND authors.nationality <> 'United States of America';
