...
ELECT COUNT(*) FROM books JOIN authors ON authors.id=books.author_id WHERE authors.nationality = 'Argentina' AND books.publication_date > 1930;
...
SELECT DISTINCT authors.* FROM books JOIN authors on authors.id=books.author_id WHERE books.title LIKE 'N%';